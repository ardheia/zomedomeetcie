...
QString heliyourte::generalTraverse()
{
    ...
    resu += "<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+QObject::tr("Il y a")+" "+str.setNum(hel_nbrePans)+" "+QObject::tr("traverses. Les points notés V sont sur la face supérieure, les points notés W sont sur la face de derrière.Les coordonnées des points W ne sont donnés que dans le cas où ils diffèrent de celles de leur point correspondant en V")+".<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+QObject::tr("Précisons que lorsqu\'on regarde les traverses depuis l\'intérieur de l\'héliyourte, le côté gauche est coupé à  l\'équerre, alors que le côté droit est coupé légèrement en biais, avec un angle de coupe (réglage scie) de ")+str2.setNum(degres(hel_angleDivision),'f',1)+" ° </p>";
...
QString heliyourte::PoteauMontantA()
{
    ...
    resu += "<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+QObject::tr("Il y a")+" "+str.setNum(hel_nbrePans)+" "+QObject::tr("montants A. Les coordonnées des points sur la face qui va être à l\'extérieur du poteau sont notés V, ceux sur la face interne W.Les coordonnées des points W ne sont donnés que dans le cas où ils diffèrent de celles de leur point correspondant en V")+".<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+QObject::tr("Précisons que les 3 trous de ce montants sont taillés en biais, avec un angle de")+" "+str3.setNum(hel_DdansA.angleBiais,'f',1)+" °.</p>";
